...
Group 8 chose to include three Naive Bayes models to compare against the logistic and random forest regression models. Naive Bayes models have a reputation for performing relatively well with noisy data, so this different approach might yield effective predictions despite varied data. Naive Bayes classification models may be compatatively sensitive to class imbalance, which is clearly present in these data. The first two models will approach different methods with the existing class imbalance and the third will up-sample the target = 1 class to minimize the effect of te class imbalance. 
...
#plots probability distributions
plot(nb1)

#predicted class holds predic values from nb1 model
nb1_predict_class <- predict(nb1, newdata = t_train)
nb1_predict_test <- predict(nb1, newdata = t_test)
...
cm_test <- confusionMatrix(data <- nb1_predict_test, reference = t_test$TARGET)

#outputs confussion matrices
print(cm)
print(cm_test)
```
### NB Model 2
...
The second model appears to perform slightly better, based on the non-negative kappa value on the test set, but it is still failing to predict any instances where TARGET = 1. We will need to address the class imbalance directly, using the same model tuning approach. This final NB model will apply the parameters of model 2 to an upsampled training dataset.

```{r nb3}
# train model using the same caret tuning and cross validation approaches as before
nb3 <- train(
